...
fn plugin_install_dir() -> Result<PathBuf, String> {
    Ok(openclaw_home()?.join("plugins").join("notemd"))
}
...
fn install(force: bool) -> Result<(), String> {
    ...
    match write_notemd_settings(&token) {
...
fn write_notemd_settings(token: &str) -> Result<PathBuf, String> {
    ...
    if cfg_path.exists() {
        ...
        if let Some(plugins) = cfg.get_mut("plugins").and_then(|v| v.as_object_mut()) {
            if let Some(entries) = plugins.get_mut("entries").and_then(|v| v.as_object_mut()) {
                changed |= entries.remove("notemd").is_some();
            }
        ...
        if let Some(channels) = cfg.get_mut("channels").and_then(|v| v.as_object_mut()) {
            changed |= channels.remove("notemd").is_some();
        }
        if changed {
            ...
            println!("✓ removed notemd entries from {}", cfg_path.display());
        } else {
            println!("(no notemd entries in {})", cfg_path.display());
        }
    }
...
fn status() -> Result<(), String> {
    ...
    if cfg_path.exists() {
        ...
        let entry = cfg.pointer("/plugins/entries/notemd");
        let channel = cfg.pointer("/channels/notemd");
        let token = cfg
            .pointer("/channels/notemd/accounts/default/accessToken")
            .and_then(|v| v.as_str());
        println!("openclaw.json : {}", cfg_path.display());
        println!("  plugins.entries.notemd   : {}", display_value(entry));
        println!("  channels.notemd          : {}", display_value(channel));
        println!(
            "  access token               : {}",
    ...
    let sock = openclaw_home()?.join("notemd.sock");
...
fn merge_config(plugin_dir: &Path) -> Result<String, String> {
    ...
    entries.insert("notemd".into(), json!({ "enabled": true }));
    ...
    let notemd = channels
        ...
        .or_insert_with(|| json!({}))
        .as_object_mut()
        .ok_or_else(|| "channels.notemd must be object".to_string())?;
    notemd.insert("enabled".into(), json!(true));
    let accounts = notemd
        .entry("accounts")
        .or_insert_with(|| json!({}))
        .as_object_mut()
    ...
    default.entry("socketPath".to_string())
        .or_insert_with(|| json!("~/.openclaw/notemd.sock"));
