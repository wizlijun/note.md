...
## 7. 重命名与改名联动（原暂缓项,2026-07-10 补做）

- FolderView 右键菜单新增**重命名**(仅文件,不含目录):行内编辑文件名,
  Enter 提交 / Esc 取消;目标名经 `sanitizeFileName` 约束;目标已存在 →
  toast 报错中止,**绝不覆盖**。
- **改名联动(硬性规则)**:对 `xxx.md` 重命名为 `yyy.md` 时,同一操作内
  同步重命名配对的 `xxx.note.md`(含旧后缀 `.notes.md`)→ `yyy.note.md`,
  防批注孤儿;伴生改名失败则**回滚**主文件改名(尽力而为的原子性)。
  直接重命名 `.note.md` 只改自身(它因此成为/脱离伴生关系,由从属推断
  规则自然决定)。
- 已打开 tab 的路径联动:重命名的文件(或其伴生)在 tabs 中打开时,
  更新 `tab.filePath`/`title` 并重绑 file-watcher,不打断编辑。
- 明确不做:入链 `[[title]]` 的自动改写(file-over-app:链接按文件名解析,
  改名后断链与其他工具行为一致,由碰撞/未解析提示兜底)。
