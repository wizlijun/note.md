...
在核心交互完成后，又补齐了与现有能力匹配的 Huabu 风格操作外壳：

- 常驻操作改为底部居中的图标工具坞，包含选择、平移、套索、锁定、新建节点、框组、粘贴和撤销/重做；快捷键、标题和 ARIA 契约保持不变。
- 节点、多选和连线操作改为随选区出现的上下文浮条；提供复制/剪切、层级、对齐/分布、颜色、分组编辑、连线标签与端点等实际可用操作。
- 节点卡片增加色彩层次、悬停/选中阴影、顶部强调线、文件/链接图标容器、分组玻璃标签和更明确的连接点反馈。
- 上下文浮条使用画布内绝对定位，随 viewport 与选区移动并受画布边界裁切，避免侧栏变化后遗留在错误的窗口坐标。
- 图标由 Svelte 内联 SVG 组件提供，没有增加 React、React 19 或新的 UI 依赖。

视觉层仍遵守原范围：没有添加 AI、Sketch、Question、图层树、跨画布引用、服务端节点或其他 Huabu 业务入口，也没有展示当前文档模型无法兑现的空按钮。

补齐后的验证结果：Canvas 定向测试 63/63；当前集成工作区全量 Vitest 251/251 文件、2,665/2,665 测试；`svelte-check` 0 error（43 个仓库既有 warning）；生产构建及 Editor Kit v1/v2 build check 通过。
