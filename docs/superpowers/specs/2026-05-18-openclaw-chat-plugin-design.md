...
3. **mdrelay** —— mdeditor 仓库内新增的 Cloudflare Worker（与 `worker/` 同级），做 WSS 中继与 pairing
...
- mdshare worker (作为 mdrelay 的参考): `worker/` (Cloudflare Worker 源码) + `mdshare/` (Rust CLI 客户端)
