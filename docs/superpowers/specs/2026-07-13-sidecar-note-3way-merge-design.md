...
Desktop 侧已依赖 `git2`(vendored libgit2,见 `src-tauri/Cargo.toml`)。使用
**`git2::merge_file(ancestor, ours, theirs, opts)`**:

- 返回 `MergeFileResult`,提供 `automergeable()` 判定与 `content()` 合并文本;
- 冲突时 `content()` 内含标准冲突标记,标记标签可通过 `MergeFileOptions`
  / `MergeFileInput` 的 label 设为可读的 `local` / `vault`;
- 零新依赖,与现有 vault 同步共用同一套库,风格一致。

被否决的备选:`diffy` crate(需新增依赖)、libgit2 完整 index merge(对单文件过重)。
