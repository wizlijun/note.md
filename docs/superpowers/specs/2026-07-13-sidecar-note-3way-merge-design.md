...
## 已知边界(实现后记录,2026-07-13)

- **三方合并主要经 `apply_update` 生效。** `sotvault_sync_to_vault` 用 `dedup_target`
  每次挑一个磁盘上不存在的新路径(`foo.md` 占用→`foo-2.md`),所以它按 `vault_path`
  查旧 `note_merge_base` 通常 miss(base=None),等于"新建副本 + 建立 base",本就
  不覆盖既有 vault 笔记。真正会覆盖既有 vault 笔记的是 `sotvault_apply_update`
  ——它按已跟踪的 `Record` 复用同一 `vault_path` 并复用其伴生笔记,这条通道现在
  完整走 base→三方合并→双侧收敛+`.conflict` 备份。这正是本设计要修的路径。
- **升级后首个 update 的存量配对:** 老 `Record` 无 `note_merge_base`(=None),若两侧
  已分叉则走"无 base 迁移"分支=保守冲突(插标记 + 双 `.conflict` 备份),不静默覆盖。
  这是一次性行为,之后 base 正常推进。
- **前端 toast 监听器为启动期单例,不解绑**(App 生命周期常驻),与 `vault.svelte.ts`
  的既有模式一致。`.conflict.<秒级时间戳>` 同秒二次冲突会覆盖前一份备份(概率极低,
  与 `vault_sync/conflict.rs` 同粒度)。
