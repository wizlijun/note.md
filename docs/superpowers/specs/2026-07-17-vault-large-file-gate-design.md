...
职责单一:检测工作区里超阈值的待提交文件。**阈值来自 vault 级配置**(见 E),
默认 10 MB。因为 `detect_oversized` 拿得到 repo 根(= vault root),直接
`crate::sotvault::vault_settings::read(repo)` 读阈值,**无需把 AppHandle 传进后台
同步线程**。
...
/// 读 vault 配置解析出的有效阈值(字节)。缺省/非法 → 10 MB。
...
pub fn detect_oversized(repo: &Path) -> GitResult<Vec<String>> {
    ...
    // 对每个路径 std::fs::metadata(repo.join(path)).len() > threshold 则收集
}
```

要点:
- 阈值判定用严格大于(`>`):正好等于阈值不算,阈值 + 1 字节才算。
...
- `VaultSyncStatus`(`vault_sync/mod.rs`)新增 `skipped_large_files: Vec<String>`,
  `vault_sync_status` 命令带出(供未来消费者;当前无桌面前端读取)。
- **不做桌面前端 toast**。实现时发现:`src/lib/vault.svelte.ts`(`VaultStatusFromRust`/
  `has_conflicts`)是 **iOS 专用**路径(`vault_ios` libgit2,不走本门禁),桌面**没有任何
  前端消费 `vault_sync_status`**。桌面 vault_sync 是后台线程 + 托盘,大文件告警完全由
  **托盘黄灯 + 大文件子菜单 + WARN 日志**承载(见 B 节)。因此原计划的
  `vault.largeFileSkipped` toast 与 `VaultSettingsTab` 按钮禁用**均已取消**——门禁不适用
  iOS,桌面"立即同步"的禁用在托盘侧完成(C4)。

### D2. i18n key

- 托盘(`lib.rs` 内 en/zh/ja/de 元组表):
  `tray.largeFiles.title`("⚠️ {n} file(s) too large" / "⚠️ {n} 个文件过大" / …)、
  `tray.largeFiles.header`("Over the limit — not synced. Move out of the vault:" / …)。
- 前端(自研 i18n,`src/lib/i18n/*.ts` 扁平点分键):
  `vaultSync.largeFileThreshold`(设置里的阈值输入 label,见 E4)。
- **不新增** `vault.largeFileSkipped`(桌面无 toast 通道,见 D1)。

---

## E. 阈值配置(vault 级,随 git 同步)

阈值是 **vault-wide 策略**(所有设备遵守同一规则),存进现成的
`{vault}/.notemd/settings.json`(`VaultSettings`,随 git 同步),默认 10 MB,
可在设置里改。复用现有 `notemd_vault_settings_get/set` 通道。

### E1. 扩展 `VaultSettings`(`sotvault/vault_settings.rs`)

```rust
pub struct VaultSettings {
    // 现有字段 sync_dir / wikipage_dir / dailynote_dir ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub large_file_threshold_mb: Option<u32>,
}
```

`merge()` 增加 `large_file_threshold_mb: Option<u32>` 形参:提供时校验 `> 0`
(0 或缺省 = 用默认 10),写入 `out.large_file_threshold_mb`。

### E2. 后端命令(`sotvault/mod.rs`)

`notemd_vault_settings_set` 增加 `large_file_threshold_mb: Option<u32>` 形参,
透传给 `merge`。`notemd_vault_settings_get` 无需改(结构体已带新字段)。

### E3. 前端 store(`src/lib/vault-settings.svelte.ts`)

- `VaultSettingsDto` 加 `largeFileThresholdMb?: number | null`。
- `vaultSettings` state 加 `largeFileThresholdMb: number`,`loadVaultSettings` 读取
  (缺省 `DEFAULT_LARGE_FILE_THRESHOLD_MB = 10`)。
- 新增 `saveLargeFileThreshold(mb: number)`:invoke `notemd_vault_settings_set`
  带 `largeFileThresholdMb`,更新 state。**无需** `refreshSotvault`(阈值不影响
  vault 目录结构;下一轮后台同步会自己读到新值)。

### E4. 设置 UI

在承载 vault 级设置(syncDir 那块)的组件里加一个数字输入项"大文件阈值 (MB)",
默认 10,`onchange` 调 `saveLargeFileThreshold`。校验 `>= 1`。i18n key
`vault.largeFileThreshold`。

### E5. 与门禁的衔接

`large_files::resolve_threshold_bytes` 每轮读 `.notemd/settings.json`,
所以改完设置**下一轮同步即生效**,无需重启同步循环。
...
1. 往 vault 丢一个 >10MB 文件(含一个中文名的)→ 托盘转黄、子菜单出现该文件、
   点击在 Finder 选中;其余小改动照常同步进 commit,大文件不进。
2. 挪走该大文件 → 下一轮托盘恢复绿、子菜单消失。
3. 设置里改阈值(如 5MB)→ 丢 6MB 文件被拦;改回 10 → 不再拦;`.notemd/settings.json`
   有 `largeFileThresholdMb`。
4. 运行时"选文件夹"配置一个新 vault → 立即自动开始同步(无需"开始同步")。
5. 同步进行中 → 托盘"立即同步"置灰;同步结束恢复。
6. 黄图标观感确认。
...
后端:
- `src-tauri/src/vault_sync/large_files.rs`(新增,阈值读 vault_settings)
- `src-tauri/src/sotvault/vault_settings.rs`(VaultSettings 加 large_file_threshold_mb + merge)
- `src-tauri/src/sotvault/mod.rs`(notemd_vault_settings_set 加形参)
...
前端:
- `src/lib/vault-settings.svelte.ts`(largeFileThresholdMb DTO/state/saveLargeFileThreshold)
- `src/components/SettingsDialog.svelte`(vaultSync 段新增大文件阈值输入项)
- `src/lib/i18n/{en,zh,ja,de}.ts`(`vaultSync.largeFileThreshold`;托盘键在 lib.rs)

> 注:原计划的 `src/lib/vault.svelte.ts`(iOS 专用)toast、`VaultSettingsTab.svelte` 按钮禁用、
> `vault.largeFileSkipped` 键均在实现时取消(见 D1)——桌面告警只走托盘。
