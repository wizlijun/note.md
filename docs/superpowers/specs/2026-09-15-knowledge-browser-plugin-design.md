...
| 项目 | 约定 |
| --- | --- |
| 规格版本 | 1.4；2026-09-18 增加默认 Graph 图，并保留 3.1.1 / 3.1.0 / 3.0.0 读取边界 |
| 插件中文名 | 提取知识浏览器 |
...
1. 主界面默认采用**全局 Graph + 节点详情**；切换到阅读后使用**结果列表 + 阅读详情**双栏，筛选与证据占用阅读区域，避免长期并列四五个信息区。
2. 五类知识对象与关系均可独立阅读。内部初选第一条主张供“当前记录”范围和阅读视图使用；没有主张时初选全部知识中的第一条。
3. Graph 是默认阅读方式；多方关系以有角色的关系节点表达，并可切换全局或当前记录两跳范围。
...
第一次打开：全局 Graph、无筛选；内部按原始数组顺序初选第一条主张，供右侧详情、“当前记录”范围和打开阅读视图使用；没有主张时初选全部知识中的第一条。Graph 投影稳定保留当前中心、非空节点类型和代表性关系类型；切到阅读后使用主张分类、全部重要性及原有确定性排序。
...
插件 3.2.0 以 `knowledge-representation-dataset/3.1.0` + `relation-schema-extractor/3.1.1` 为当前格式，同时兼容同一 Schema 下的 3.1.0 规则及成对的 3.0.0 schema/rule；各版本都要求关系表 `1.0.0`。读取 3.1.0 规则时给出非阻断兼容提示，仍默认进入 Graph 图。交叉配对、2.x、未知版本和未知注册表版本显示“尚不支持此版本”，可查看原始 JSON；不得猜字段、自动迁移或用最新版注册表替代旧版本。插件遵循自身 SemVer，可因浏览能力升级而独立递增；提取规则升级不要求虚构新的数据集 Schema 版本。
...
**KB-GRAPH-01**：默认展示全局 Graph；用户可切换为“当前记录”，以切换瞬间的选中对象为中心展示两跳邻域。两跳按图上的实际连接计算；关系实例仍以独立节点出现。两种范围都使用确定性投影，不把布局坐标写回数据。
...
**KB-GRAPH-03**：节点文字来自确定性标题解析。Graph 只把实体、概念、主张、事件、叙事和关系六类知识记录作为节点；来源与证据留在阅读详情中，不能伪装成已消歧实体。`about`、事件参与、叙事成员和关系引用作为始终可见的虚线结构引用层，并在图例和边详情中明确标为“引用”，与实线关系角色分开。

**KB-GRAPH-04**：单次渲染最多 80 个对象节点与 120 条连接线；关系节点计入节点上限。按 Priority 与原始顺序稳定选择完整关系组，并尽量保留每种非空节点与关系类型。关系组必须整组进入或整组省略；若一条关系本身超限，或参与者包含 Graph 不设节点的来源，不在画布中显示该关系，用户仍可在关系组/阅读视图查看完整角色表，不能截掉一部分参与者后冒充完整关系。界面显示被省略的节点和边数量。

**KB-GRAPH-05**：点击节点与右侧详情及应用选择同步；点击连接线查看类型、角色与两端记录。缩放、平移、拖动、适配画布和按名称跳转均可用。键盘用户可以通过等价连接列表选择边，并可从详情打开完整阅读视图。
...
| 知识 Graph | 规定上限内布局及首帧 ≤ 800 ms；范围切换不冻结详情阅读 |
...
大型文件不得在主线程同步完成全部解析与索引；使用同源打包 Worker，并验证宿主 CSP 允许该 Worker。列表使用虚拟化，原始 JSON 按当前对象显示；进入宿主 Source 视图视为用户显式触发全文格式化。宿主只为严格合法的 `.json` 调整 token 之间的空白并使用两空格缩进，不得经对象重序列化改写大整数、转义、重复键或键顺序；无效 JSON、JSONC、超过 32 MiB 字符或 128 层嵌套的输入保持原文。格式变化作为正常未保存修改，可由用户保存或放弃。
...
| C：完成探索与发布质量 | Graph 全局/两跳范围、缩放拖动、窄屏、主题与 Chromium 生产包已自动化验证；大数据虚拟化、p95 基准与真实 WKWebView 人工验收待完成 | 阶段 C 交付物完成，第 15 节全部 MUST 验收完成并记录 |
...
| AC-41 | 打开含六类对象、多类关系和三方参与者的数据集 | 默认 Graph 图同时区分六类节点；关系以独立枢纽保留全部角色，不转写成两两事实边 |
| AC-42 | 在 Graph 图切换全局/当前记录、选择节点，并拖动或缩放 | 画布不因选择详情而重建；邻域只保留相连对象和关系，可随时适配并切回其他视图 |
| AC-43 | Graph 图达到 80 节点或 120 边上限 | 当前中心、各非空对象类型和代表性关系类型优先保留；界面明确显示截断数量且保持可操作 |
