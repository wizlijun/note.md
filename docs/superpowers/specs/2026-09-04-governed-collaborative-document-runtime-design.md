...
为避免一次性固化未经验证的接口，按以下有独立闸门的小切片依次交付：
...
- 先接入既有 Claim 的精确只读 projection，再实现 propose-replace、durable intent、outbox 和 reconcile。
- Claim revision/hash、truth、consent、有效期和审核状态继续唯一属于 Memory v2。

**Stage 1D：用途与迁移闭环**

- 单一 `agent-context` Publication、Annotation、Assessment 统一 ChangeSet、当前业务 bundle 和真实 Agent 生命周期入口。
