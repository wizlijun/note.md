...
- 日期：2026-09-19
- 最后更新：2026-09-20
- 状态：首个可用实现切片已完成；正式协议为 v1，后续能力边界见下文
- 产品中文名：沟通词典
...
沟通词典维护用户本人参与沟通时，ASR 容易识别错误的人名、组织、产品、项目和专用术语。Agent 提出疑似错误，用户确认正确写法；后续处理同一沟通场景的转写时复用确认过的规则。
...
- 正确写法与常见误识别是核心数据；不维护口语教学、正确读法、发音训练或声纹。
...
“历史”和“设置”是次级入口。待确认增加“历史整理”批次分组，展示生成 Skill 的覆盖、候选聚合和冲突；详见 [存量生成 Skill 与数据集设计](2026-09-19-conversation-dictionary-dataset-skill-design.md)。待确认按场景筛选；同一误识别的多次出现可聚合展示，但每个来源保留自身范围判断，不能用一次参与证明覆盖其他来源。
...
- 人先选择建议或自行填写正确写法；“确认”在决定完整后可用。建议默认仅纠正这次；用户主动选择保存规则时，额外显示场景与后续使用方式。修改直接在同一表单完成。
- 候选排序、置信度与 Agent 理由只读；用户编辑的是最终决定。
- 场景、词条类型、正确写法、误识别形式、说明和并入目标可修改；改变场景后重新计算歧义。相同名字不自动并入同一词条。
...
- UI 显示“场景、正确写法、常见误识别”等用户语言，不将 `domain`、`canonical`、`alias` 作为标签。

### 2.4 词典、历史与错误

词典支持搜索、按场景和类型筛选、新建与编辑、启停规则。直接点击词条名称进入编辑，显示其正确形式、各场景规则与最近确认时间。删除仍被规则引用的正确形式或词条须先处理引用，不能静默影响所有场景。场景间不隐式继承规则；多个场景通过各自的规则引用同一词条。词条是写法条目，不发展为联系人或关系图谱；V1 不做场景树。
...
| 数据 | 默认路径 | 内容 |
| --- | --- | --- |
| 可移植词典 | `ssot/meetings/conversation-dictionary.yml` | 场景、正确写法、误识别与保留规则 |
...
共享词条，逐场景批准规则。多种正确写法不自动互相转换，修改 label 不改变规则输出。
...
- entry 的 kind 为 `person | product | organization | project | acronym | technical_term | other`；label 用于显示，forms 为非空正确字符串列表，按 NFC 去重；description 仅提供一般消歧背景。
- replacement rule 必须有明确的 domain_id、observed、target.entry_id、target.text；输出必须属于目标 entry 的 forms。observed 与输出不能相同。
...
- 同场景同 observed 指向不同 entry/输出是局部语义歧义，可以存储并展示，但不自动应用。不会使其他已验证场景失效。
...
2. 比较用 NFC，大小写敏感，不折叠空白或做 NFKC。正确 forms 本身不构成替换规则或全局保护；大小写变体需明确规则。
3. 匹配完整 observed 词组，禁止无边界子串替换。实现须固定分词依赖与版本；无法证明的中文边界返回不确定，而不是只用正则 `\b` 硬替换。
4. preserve 对命中区间优先，阻止重叠 replacement；不同位置不互相影响。
5. 同位置多个 entry/输出目标构成歧义，suggest 规则也参与冲突检测。两个同名 entry 即使输出文字相同，也不能据此自动合并身份。
...
- `existing_entry_id` 如给出，必须指向共享 entries 中的既有条目。output 不在现有 forms 中时要求人工明确添加正确形式或新建条目，不能静默改名或合并。
...
历史整理允许用户选择并集中批准已展示的变更计划：每条规则仍有独立选择与结果，依赖同时展示；未解决冲突、未知来源、合并既有词条和停用旧规则须先明确处理。一次提交整个计划、revision 只增加一次，Agent 不可批准。直接手工新建词条/规则也经过相同人工提交服务，可不关联候选或伪造转写来源。编辑 label、forms、target、application、场景以及规则启停都有历史，停用一条映射不影响其他场景规则。
...
用户在“历史整理”界面查看原文证据、修改正确写法并保存选中的变更；未解决冲突保持在独立提示区，不会随选中提案写入。完整来源覆盖、提案类型、依赖、批次 CAS 与重跑幂等见 [存量字幕生成 Skill 与待审数据集](2026-09-19-conversation-dictionary-dataset-skill-design.md)。
...
3. 将领域转为 domains、正确写法转为 entries、每个旧 alias 转为所属场景的独立 rule 草稿；跨场景同名 entry 不自动合并；展示完整 diff，提示新用途仅为本人参与沟通的 ASR 校正。
4. `spoken_forms` 不进入 V1，必须在差异中列明；旧 preserve 转为无 target 的独立规则。无别名 replacement 只导入正确形式，不制造规则；冲突与类型错误进入待补全草稿。
...
| 场景 | 不同场景的同 alias 可对应不同 canonical；无场景、错误场景 ID、歧义场景不回退全库 |
| 匹配 | NFC/NFD 对照、大小写、英文词内子串、中文同名前后缀、多正确形式、改 label 不改输出、标点、emoji、重叠和 preserve 均有固定 fixture；位置能回指原文 |
