...
## 11. 公共场景、移动与合并（2026-09-20）

- 内建 `public` 场景在界面始终显示为 `public（公共）` 并置于首位。没有任何场景归属的词条自动显示在这里；读取旧词典不改写其字节或审核基线。public 只在调用者明确选择该场景时参与查询，不是通配、继承或全库回退。
- 正式词典 v1 增加可选 `entry_domains: {entry_id: [domain_id, ...]}`。有效归属为这个映射与引用该词条的 replace 规则场景的并集，空集合解释为 public。旧文件省略映射仍可读；只有正式名、没有别名和规则也能保存，保存归属无需制造无操作规则。
- 拖放词条到左侧场景，或使用编辑区“移动到场景”，移动当前场景归属及相应规则；其他共享场景保持。规则的启用状态和应用方式不因移动升级。移动到已有归属时收敛归属；遇到冲突原子拒绝。同一场景拖放不提交。
- “保存”之后的“合并到…”打开目标词条选择区，可按正式名或别名搜索，目标显示类型和场景以帮助区分同名实体。确认后保留目标 ID、正式名、类型，合并别名与归属，来源正式名保留为别名，规则输出全部指向目标正式名，来源词条删除。
- 合并去除无操作规则；同一场景/观测/目标的重复规则只做保守合并，不把 suggest 升为 automatic、不把禁用规则开启。来源旧正式名需要新增归一规则时只创建 suggest。与其他词条或 preserve 的冲突拒绝提交。
- 移动和合并经 UI 明确操作提交，携带 transaction ID、词典 revision 和 SHA，复用锁、journal、恢复副本与审核记录。失败时保留原词典及编辑草稿。未保存修改在切换/移动/合并前要求用户确认处理。
