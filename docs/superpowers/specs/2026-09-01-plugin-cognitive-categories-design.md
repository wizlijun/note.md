...
插件一级分类按用户完成插件主流程后获得的主要认知结果划分。AI 是跨分类的协作方式，不作为第七个一级分类。
...
| 创作 | `notemd.md2pdf`、`notemd.power-mode` |
...
1. 新 manifest 直接写六类稳定键。
2. 旧键按最接近的新类降级：`agents → advance`、`capture → record`、`thinking → reflect`、`import-export → create`、`editing → create`。
...
- 组件测试固定 AI 精选区、角色徽标、旧缓存迁移和六类 DOM。
