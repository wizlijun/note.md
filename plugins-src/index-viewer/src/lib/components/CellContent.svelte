<script lang="ts">
  ...
  let { cell, open }: { cell: IndexCell; open: (href: string, kind?: 'page') => void } = $props()
  const parts = $derived.by(() => {
    const result: { text: string; href?: string; kind?: 'page' }[] = []
    let position = 0
    for (const link of cell.links) {
      if (link.start > position) result.push({ text: cell.text.slice(position, link.start) })
      result.push({ text: cell.text.slice(link.start, link.end), href: link.href, kind: link.kind })
      position = link.end
    }
...
{#each parts as part}
  {#if part.href}<button type="button" class="cell-link" onclick={() => open(part.href!, part.kind)}>{part.text}</button>{:else}{part.text}{/if}
{/each}
<style>
