...
export async function commitEntry(
  ...
  const final_raw_abs = await request<string>("fs_atomic_copy", {
  ...
  await request("fs_rename_strict", { src: tmp_md, dst: `${sot_book_dir}/book.md` });
  ...
  await request<string>("write_text_file", {
