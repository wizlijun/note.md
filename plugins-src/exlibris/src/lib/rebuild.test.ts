...
describe("applyRebuildDiff", () => {
  ...
  it("renames each book dir using rename_strict", async () => {
    ...
    // Mock: every request call returns appropriate values for rename + read + write
    vi.mocked(request).mockImplementation((cmd: string) => {
    ...
    expect(request).toHaveBeenCalledWith("fs_rename_strict", {
