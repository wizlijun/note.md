...
日期取创建时间的 UTC 日期，不会随着修改改变；slug 保留中文，英文转小写，用连字符分隔。同一层级出现同名账户、文件夹、同日同标题笔记或同名附件时，使用稳定的 `-2`、`-3` 序号，避免覆盖且不因枚举顺序改变。修改标题、移动笔记时更新路径及附件链接。

同步器使用的完整账户、文件夹、笔记和附件 ID 只保存在 `applenotes/id-sync.json`。该文件同时记录目标路径和内容摘要，供同步器识别改名、移动、删除、本地冲突及暂时无法重新导出的旧附件；不要手动修改或删除。
...
- 系统公开接口中的笔记消失后，对应的受管理文件会移入 `.notemd/apple-notes/trash/<transaction>/`；更新替换的旧文件也保存在这里，可手动复制到其他目录恢复。新回收路径不使用来源 ID，回收区不会自动清空。
- 从 1.0.1 迁移时，隐藏回收区中的旧版恢复副本可能仍含当时的 ID YAML；它们不参与新映射，也不属于日常浏览目录。现役镜像与后续回收副本均不再生成这类 YAML。
- 唯一长期同步清单和中断恢复状态位于 `applenotes/id-sync.json`；`.notemd/apple-notes/` 只保留跨进程锁、暂存、旧版迁移标记和回收内容。中断后下次正式同步会先完成恢复。
