<script lang="ts">
  import { describeDelta } from './domain'
...
<div class="scrim" role="presentation" onclick={(event) => event.target === event.currentTarget && !busy && oncancel()}>
  <div class="sheet" role="alertdialog" aria-modal="true" aria-labelledby="confirm-title" aria-describedby="confirm-description" tabindex="-1">
    ...
    <dl>
      <div><dt>优先级</dt><dd>{proposal.proposal.suggested_priority ?? '继承'}</dd></div>
      <div><dt>方向</dt><dd>{proposal.proposal.suggested_polarity ?? '继承'}</dd></div>
      <div><dt>证据性质</dt><dd>{proposal.proposal.suggested_epistemic_status ?? '继承'}</dd></div>
      <div><dt>确定度</dt><dd>{proposal.proposal.suggested_certainty ?? '继承'}</dd></div>
    </dl>
    <footer>
...
<style>
  ...
  dl { display:grid; grid-template-columns:1fr 1fr; gap:8px 18px; margin:14px 0 0; }dl div { display:flex; justify-content:space-between; gap:12px; }dd { margin:0; font-size:12px; font-weight:600; }
