//! Unix-domain-socket client to the local OpenClaw host. Ported from
//! src-tauri/src/openclaw/uds_client.rs (already tauri-free — it only produces
//! UdsEvents on an mpsc channel; the reader/forwarder lives in lib.rs).
...
use crate::protocol::{unwrap_from_wire, wrap_for_wire, Frame};
