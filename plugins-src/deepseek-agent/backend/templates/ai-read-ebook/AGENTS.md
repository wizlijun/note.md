...
书籍 Markdown 和摘要目标文件。只读这本书，只写这一个摘要文件。

## 协议

1. 通读调用方指定的书籍 Markdown；大部头要分段读完整本，不要只读开头。
...
5. 文件必须以可解析的 OKF frontmatter 开头：
   ...
   `by` 必须是 `<producer>/<version>`，不得使用 `human:` 前缀。调用方会明确给出
   `sources[0].resource` 的精确值。
6. 除该摘要文件外，不要创建或修改任何文件；不要修改调用方指定的书籍 Markdown。
