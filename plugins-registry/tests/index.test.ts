...
describe('GET / (landing page)', () => {
  it('serves the HTML marketplace page with html content-type + CORS', async () => {
    ...
    expect(body).toContain("group_ai:'AI'")
    expect(body).toContain('System Feature')
    ...
    expect(body).toContain("'notemd.power-mode':'experience'")
    expect(body).toContain("'notemd.conversation-dictionary':'ai'")
    expect(body).toContain("'notemd.assistant-mail':'record'")
    expect(body).toContain("'notemd.meetings':'record'")
    expect(body).toContain("'notemd.apple-notes':'import-export'")
    expect(body).toContain("'notemd.index-viewer':'reading'")
    expect(body).toContain("'notemd.knowledge-browser':'reading'")
    expect(body).toContain("'notemd.typst-reader':'reading'")
    expect(body).toContain("'notemd.memory':'reflect'")
    expect(body).toContain("'notemd.roam-import':'import-export'")
    expect(body).toContain('Sync Roam Research…')
    expect(body).toContain('同步 Roam Research…')
    expect(body).toContain('Conversation Transcript Corrections…')
    expect(body).toContain('沟通转写勘误…')
    expect(body).not.toContain('Conversation Dictionary…')
    expect(body).not.toContain('沟通词典…')
    expect(body).toContain('Assistant Mail…')
    expect(body).toContain('Sync Apple Notes…')
    expect(body).toContain('Open Knowledge Browser…')
    expect(body).toContain('Meetings…')
    expect(body).toContain('Open Typeset Reader')
    expect(body).not.toContain('→ Import from Roam Research…')
    expect(body).not.toContain('→ 从 Roam Research 导入…')
    expect(body).toContain("'notemd.codex-agent':{en:'AI Action',zh:'AI 执行'}")
    expect(body).toContain("'notemd.memory':{en:'AI Memory',zh:'AI 记忆'}")
    expect(body).toContain("'notemd.conversation-dictionary':{en:'AI Review',zh:'AI 审阅'}")
    expect(body).toContain("if(values.indexOf('universal')>=0)return d.platform_all")
    expect(body).toContain("arch==='aarch64-apple-darwin'")
    expect(body).toContain("arch==='x86_64-pc-windows-msvc'")
    expect(body).toContain("platform:'Platforms: '")
    expect(body).toContain("platform:'支持平台：'")
    expect(body).toContain('native backends, when present, run out of process')
    expect(body).not.toContain("each plugin's process is its own")
    expect(body).not.toContain('每个插件独立进程运行')
    expect(body).toContain('<link rel="canonical" href="https://plugins.notemd.net/">')
    expect(body).toContain('<meta property="og:title" content="note.md plugins">')
    expect(body).toContain('<meta name="twitter:card" content="summary">')
    expect(body).toContain('<a href="?lang=en" data-lang="en">EN</a>')
    expect(body).toContain('<a href="?lang=zh" data-lang="zh">中文</a>')
