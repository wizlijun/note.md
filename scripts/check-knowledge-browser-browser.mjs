...
try {
  ...
  await check('valid fixture completes the real file_view.ready handshake without changing source bytes', async () => {
    ...
    await plugin().getByText('strong_only · ≥ strong', { exact: true }).waitFor()
    await plugin().getByText(/证据强度/).waitFor()
    await plugin().getByRole('heading', { name: '知识图谱', exact: true }).waitFor()
    await plugin().getByRole('img', { name: '多类型知识节点与关系交叉图' }).waitFor()
    await plugin().getByLabel('关系类型').getByText('delegates_to', { exact: true }).waitFor()
    await plugin().getByLabel('关系类型').getByText('requires_approval', { exact: true }).waitFor()
    await plugin().locator('.graph-canvas canvas').first().waitFor()
    assert.equal(await plugin().getByRole('group', { name: '阅读方式' }).getByRole('button').count(), 5)
    for (const width of [320, 768, 1280]) {
      ...
      assert.ok(overflow <= 1, `${width}px graph overflow: ${overflow}`)
      await page.screenshot({ path: join(output, `graph-${width}-light.png`) })
    }
    await page.emulateMedia({ colorScheme: 'dark' })
    await page.screenshot({ path: join(output, 'graph-1280-dark.png') })
  ...
  await check('graph, reading, relation, time and diagnostics surfaces are reachable from the production view', async () => {
    await plugin().locator('.graph-canvas canvas').first().evaluate(canvas => { canvas.dataset.graphInstance = 'replace-on-scope' })
    await plugin().getByRole('button', { name: '当前对象', exact: true }).click()
    await plugin().locator('[data-graph-instance="replace-on-scope"]').waitFor({ state: 'detached' })
    await plugin().locator('.graph-canvas canvas').first().waitFor()
    await plugin().getByText(/\d+ 个节点 · \d+ 条连接/).waitFor()
    await plugin().locator('.graph-canvas canvas').first().evaluate(canvas => { canvas.dataset.graphInstance = 'preserve-on-selection' })
    await plugin().getByLabel('跳转').selectOption('e2')
    await plugin().getByRole('heading', { name: '发布执行人', exact: true }).waitFor()
    assert.equal(await plugin().locator('.graph-canvas canvas').first().getAttribute('data-graph-instance'), 'preserve-on-selection')
    await plugin().getByRole('button', { name: '全局', exact: true }).click()
    await plugin().getByRole('button', { name: '适配全图', exact: true }).click()
    ...
    await plugin().getByRole('button', { name: '关系组', exact: true }).click()
