...
# ── index-viewer: ui-only, single universal package (same shape as decision-log)
release_index_viewer() {
  local id="notemd.index-viewer"
  local src="$REPO_ROOT/plugins-src/index-viewer"
  local manifest="$src/manifest.v2.json"
  local version; version="$(manifest_field "$manifest" version)"
  echo "== $id @ $version =="

  echo "[$id] building UI bundle (pnpm --filter index-viewer build)…"
