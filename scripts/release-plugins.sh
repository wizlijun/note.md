...
#                idea-spark, next, power-mode, trace-source, timeline }   (add a case below)
...
for arg in "$@"; do
  case "$arg" in
    ...
    md2pdf|roam-import|meetings|openclaw|pos-log|decision-log|weekly-review|memory|claude-agent|codex-agent|deepseek-agent|ebook-import|idea-spark|next|power-mode|trace-source|timeline) PLUGINS+=("$arg") ;;
    -h|--help)
      grep '^#' "$0" | sed 's/^# \{0,1\}//'; exit 0 ;;
    *) echo "unknown arg: $arg (expected --release | md2pdf | roam-import | meetings | openclaw | pos-log | decision-log | weekly-review | memory | claude-agent | codex-agent | deepseek-agent | ebook-import | idea-spark | next | power-mode | trace-source | timeline)" >&2; exit 2 ;;
  esac
done
if [[ ${#PLUGINS[@]} -eq 0 ]]; then
  echo "usage: scripts/release-plugins.sh [--release] <md2pdf|roam-import|meetings|openclaw|pos-log|decision-log|weekly-review|memory|claude-agent|codex-agent|deepseek-agent|ebook-import|idea-spark|next|power-mode|trace-source|timeline>..." >&2
  exit 2
fi
...
# ── timeline: ui-only, single universal package (same shape as decision-log)
release_timeline() {
  local id="notemd.timeline"
  local src="$REPO_ROOT/plugins-src/timeline"
  local manifest="$src/manifest.v2.json"
  local version; version="$(manifest_field "$manifest" version)"
  echo "== $id @ $version =="

  echo "[$id] building UI bundle (pnpm --filter timeline build)…"
...
echo "       wrangler kv key put index --path dist-plugins/index.json --binding INDEX"
