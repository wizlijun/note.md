...
# ---------------------------------------------------------------- compare
{
 ...
 "title": "note.md vs Roam Research（2026）— 文件、agent 与持续同步",
 "desc": "一份实用对比：note.md 与 Roam Research 的大纲、每日笔记、[[双链]]、本地文件和 agent 接入，以及完整导出与持续同步路径。",
 "crumb": "对比",
 "h1": "note.md vs Roam Research",
 "lead": "两者都爱大纲、每日笔记和 [[双方括号]]。Roam 以托管图谱为中心；note.md 以你拥有的文件夹为中心。",
 "table": {
  ...
  "rows": [
   ...
   ["AI agent", "一等公民：纯文件 + <code>AGENTS.md</code>，agent 会读你的批注", "文件型 agent 需要集成、导出或 CLI 桥接"],
...
<p>Roam 把在线图谱保存在其服务中；note.md 把普通文件作为第一资料。当 agent 需要直接访问文件系统、你需要 Git 历史，或希望档案脱离原应用也能读取时，这个区别很重要。Roam 提供导出和桌面 CLI 桥接；note.md 可以利用两者，并不要求你离开 Roam。</p>
<p>note.md 保留了 Roam 的好东西——大纲编辑器、每日笔记、一个大 <code>[[namespace]]</code>、即时搜索——然后在文件之上把它重建了一遍。你的 vault 今天能在任何编辑器里打开，五十年后也能。它还补上了 Roam 从未有过的东西：你的 agent 是一等公民，动笔之前先读你的批注。</p>"""),
  ("从 Roam 同步", """<p>已发布的 <b>Roam Research Sync</b> 插件提供三条路径。完整图谱 JSON 导出会生成 <code>wikipage/</code> 大纲笔记和 <code>dailynote/yyyy/yyyy-MM-dd.note.md</code> 每日笔记，并把 <code>[[July 10th, 2026]]</code> 改写为 <code>[[2026-07-10]]</code>。每日与增量 CLI 同步通过 Roam 桌面应用和 <code>roam</code> CLI 合并后续变化，同时保留本地块。你可以继续使用 Roam，也能让选中的知识成为 agent 可搜索的本地上下文。</p>"""),
  ("怎么选", """<ul>
<li><b>留在 Roam</b>：如果块引用、嵌入和查询在你的工作流里是承重墙，而且托管图谱适合你的需求。</li>
...
},
{
 ...
 "table": {
  ...
  "rows": [
   ...
   ["价格", "免费，开源", "有免费档；付费方案按席位；AI 按方案包含或计量"],
   ["离线", "永远可用 —— 这是你的磁盘", "桌面和移动应用可离线使用选定页面"],
   ["AI", "任何 agent，经由纯文件 —— 你说了算", "Notion AI、多种受支持模型与 Notion MCP"],
...
<p>Notion 现在提供离线页面、多种 AI 模型和 Notion MCP；这些都是真实优势。区别在所有权与互换性：note.md 让任何能访问文件系统的 agent 使用同一批本地文件和 <code>AGENTS.md</code>，工具与源内容之间不需要导出步骤或工作区 API。Notion 仍更擅长协作数据库；note.md 则刻意让长期资产保持为文件夹。</p>
<p><small>Notion 能力与方案最后复核于 2026-09-22：<a href="https://www.notion.com/help/use-pages-offline">离线页面</a>、<a href="https://www.notion.com/help/notion-ai-faqs">Notion AI</a>、<a href="https://www.notion.com/pricing">价格</a>。</small></p>"""),
  ("怎么选", """<ul>
...
<li><b>常见搭配：</b>团队用 Notion，自己用 note.md。</li>
 ...
 "faq": [
  ...
  ("为什么本地优先对 AI 重要？",
   "Agent 在能直接读写的纯文件上干活最顺手。本地 markdown vault 让你直接使用已有的 Agent 或本地运行环境，不必把知识交给某一家 AI，也不会再多一层 Token 账单。实际模型用量与限制仍按你选择的服务执行。"),
 ],
},
# ------------------------------------------------------------ integrations
{
 ...
 "sections": [
  ("为什么这对组合成立", """<p>OpenClaw 把记忆存成 markdown——长期事实放 <code>MEMORY.md</code>，每日工作笔记放 <code>memory/YYYY-MM-DD.md</code>。note.md 也使用文件，但其 <code>wikipage/</code> 与 <code>dailynote/yyyy/*.note.md</code> 有不同的路径和格式。应通过明确的工作区或转换规则连接两者，不要把格式当成完全相同。</p>
...
<li>把 OpenClaw 的工作区指向 vault，或让它把报告写进专用 vault 目录。没有转换器时不要把 <code>memory/</code> 软链到 <code>dailynote/</code>：目录与元数据不同。</li>
<li>也可安装官方 OpenClaw Chat 插件，在 note.md 内使用对话窗口。</li>
...
<li>告诉 OpenClaw 做后续工作前先读sidecar 文件。你的判断成了它的方向盘。</li>
 ...
 "faq": [
  ("OpenClaw 需要插件才能和 note.md 配合吗？",
   "文件协作不需要插件：两边都能按 AGENTS.md 的边界使用纯 markdown。可选的官方 OpenClaw Chat 插件会增加 note.md 原生对话窗口。"),
  ("让 OpenClaw 写进我的 vault 安全吗？",
   "把 vault 放进 git（见 GitHub 指南），已提交的 agent 写入就可 diff、可回滚。按约定，.note.md 属于人；Smart Lookup 的 fenced answer 回写是唯一狭窄例外。把边界写进 AGENTS.md。"),
 ],
},
{
...
<li>加上 vault 专属规则——例如“除 Smart Lookup fenced answer 外，把 <code>*.note.md</code> 视为人拥有”、“新调研放 <code>research/</code> 下，文件名带日期前缀”。</li>
...
<li>在 note.md 里审读它的产出；批注；告诉下一次运行去读sidecar 文件。</li>
 ...
 "faq": [
  ("Codex 需要 MCP 服务器才能用 vault 吗？",
   "不需要。vault 就是工作目录里的纯文件——Codex 的主场。若更适合工具接口，已发布的本地只读 Vault MCP 提供 search 与 vault_info；分享 Worker 的 MCP 是另一套发布接口。"),
  ("AGENTS.md 里该禁止什么？",
   "把 .note.md 视为人拥有。Smart Lookup 的 fenced answer 回写是唯一狭窄例外；普通 agent 不应改高亮、问题或已采纳结论。其余（命名、目录、链接风格）都是各家的偏好。"),
 ],
},
{
...
<p>把 Worker URL 和 API key 粘进 <b>note.md → Preferences → Share</b> 并保存。完整细节见仓库里的 <code>worker/README.md</code>。</p>"""),
  ("你会得到什么", """<ul>
...
<li><b>为 agent 就绪：</b>Worker 暴露一个 MCP 端点，你的 agent 可以代你发布——<code>notemd share draft.md</code> 在任何脚本里都能跑。</li>
</ul>"""),
 ],
 "faq": [
  ("这要花多少钱？",
...
},
{
 ...
 "sections": [
  ("为什么 git 是 vault 的完美后端", """<p>数据库需要你总会忘记做的备份。同步服务需要订阅和信任。git 让每次<em>提交</em>成为历史、每次推送成为异地副本；保存文件不会自动提交。在 agent 时代它双倍回本——<b>agent 写进 vault 后，git 让已提交的变化可 diff、可追溯、可回滚。</b>agent 发挥失常的一天，可以是一句 <code>git revert</code>，不是一场悲剧。</p>"""),
...
<p>就这样。GitHub 私有仓库免费，历史无上限。之后想提交多勤都行——或者交给自动化。</p>"""),
  ("同步与自动化", """<ul>
<li><b>note.md 集成：</b>内置的 <b>Sync to Vault</b> 命令把文件以带日期前缀的名字拷进 git 同步的 vault，刷新时能感知冲突；最近文件历史可通过 vault 镜像。</li>
<li><b>自动提交：</b>单写入端可定时运行 <code>git add -A &amp;&amp; if ! git diff --cached --quiet; then git commit -m "auto" &amp;&amp; git push; fi</code>。无变化时会跳过；多设备仍需明确的 pull 与冲突策略。</li>
...
<li><b>agent：</b>给 agent 一个工作副本。像审同事的 PR 一样审它们的提交——因为它们现在就是同事。</li>
 ...
 "faq": [
  ...
  ("我需要会 git 吗？",
   "需要掌握 add、commit、pull、push 和冲突处理。note.md 写普通文件并提供 Sync to Vault，但不会替代 Git 的历史与冲突模型。"),
 ],
},
{
...
<li>在 note.md 里打开结果；阅读、高亮、发问——你的标记落进sidecar 的 <code>.note.md</code>，原文保持干净、可再生成。</li>
 ...
 "faq": [
  ("ChatGPT 会自动读 AGENTS.md 吗？",
   "不像 CLI agent（Codex、Claude Code）那样自动读。把 vault 约定贴进 ChatGPT 的项目或自定义指令，并指向 AGENTS.md。sidecar 由人拥有，Smart Lookup fenced answer 回写除外；新文件遵守你的命名规则。"),
...
<p>把 vault 放进 <a href="/guides/vault-on-github/">git</a>，每一次 agent 写入都可 diff、可归属、可回滚——agent 的糟糕一天，是一次 <code>git revert</code>，而不是一场事故。</p>"""),
  ("动手搭起来", """<ol>
<li>在 vault 根放一个 <code>AGENTS.md</code>——把 <a href="/llms-full.txt">llms-full.txt</a> 的 vault 规则作为参考，再加 house rules。agent 通常把 <code>*.note.md</code> 视为人拥有；Smart Lookup 的 fenced answer 回写是狭窄例外。</li>
...
<li>在 note.md 里阅读并批注结果；让下一个 agent 先读sidecar 文件。循环在你的磁盘上闭合。</li>
 ...
 "faq": [
  ...
  ("这需要专门的编排工具或 MCP server 吗？",
   "不需要中枢编排器：规则在 AGENTS.md，产出在 .md，判断在 .note.md。agent 可以直接用文件，也可用已发布的本地只读 Vault MCP（notemd mcp），它提供 search 与 vault_info。分享 Worker 的 MCP 是另一套发布接口。"),
  ("为什么不干脆用一个 AI 包办一切？",
...
},
{
 ...
 "title": "可信的个人 AI 记忆：Agent 发现，由你确认 | note.md",
 "desc": "可靠的个人 AI 记忆，始于 Agent 从日常工作中发现候选记忆，再由本人逐条确认，之后才能成为可信的上下文。",
 "crumb": "产品主张 · Memory",
 "h1": "Agent 负责发现，你负责确认。个人记忆由此变得可信。",
 "lead": "对于你从未说过的个体事实，模型无从知晓。但让你专门写一本“个人使用说明书”也行不通。note.md 选择了第三条路：让 Agent 从你带入的工作和沟通中发现重要信息，再由你逐条批准；只有这样，其他 Agent 才能把它当作可靠依据。",
 "sections": [
  ("再聪明的模型，也猜不出你的人生", """<p>大语言模型很擅长处理公共知识，也就是那些普遍成立、可以由许多人验证的事实。但个人信息完全不同。任何模型都无法仅凭互联网推断出你希望别人如何称呼你、你去年为什么否决某个方案、你在重复性工作中更喜欢哪款工具，或 Agent 绝不能越过哪条边界。</p>
<p>能力更强的模型，也许能给出听起来更像真的猜测，而这恰恰是危险所在：关于你的错误答案一旦表达得足够流畅，就可能显得非常合理，但只有你能纠正它。检索能力再强，也无法弥补基础信息的缺失。如果信息从未被记录，就没有任何内容可供检索。</p>"""),
  ("你不该被迫为自己写一本使用说明书", """<p>常见的做法是填写个人资料、自定义指令，或手动维护一份记忆文件。这些方法可以作为补充手段，却不适合成为主要入口。你熟悉的事情对自己而言只是背景，因此很难意识到哪些信息对 Agent 来说是新的。最有价值的记忆——一次决策背后的原因、失败带来的教训、不经意流露的工作偏好——往往也最难让人临时概括出来。</p>
<p>而且，人会改变。一次写好的个人资料只是一张照片，真实的人却是一段不断发展的故事。记忆产品不应该把“描述自己”变成又一个需要你长期维护的收件箱。</p>"""),
  ("有用的信息，其实已经存在于你的工作中", """<p>你可能从不会专门坐下来告诉 AI：“我是这样做决定的。”但在项目讨论、邮件、会议记录、Agent 会话，以及你编辑的文档中，这些信息会自然流露出来。它们保留了表单容易丢失的东西：第一人称表达、时间、交流对象、原因和上下文。</p>
<p>这意味着可以采用一种更合理的分工：</p>
<ol>
<li><b>你把选定的工作资料和沟通放入自己的 vault。</b> 原始内容继续作为证据保留，并始终由你掌控。</li>
<li><b>Agent 发现细小、独立的候选记忆。</b> 它每次只提出一条清晰的主张，而不是凭空拼出一份完整的个人画像。</li>
<li><b>你逐条判断。</b> 可以确认、否认、标记为重要，也可以直接忽略。</li>
<li><b>只有你的决定才能产生可信记忆。</b> 获得批准的主张会成为持久、带版本记录的资产，并可生成简洁的 <code>USER.md</code> 和 <code>MEMORY.md</code> 视图。</li>
</ol>
<p>机器负责扫描和起草，你则保留那项无法交给别人完成的工作：判断一条主张是否真正代表你。</p>"""),
  ("记录只是证据，不会自动变成事实", """<p>日常语言往往含糊不清。“我们下季度会迁移到 PostgreSQL”可能是一项决定、一份提议、一个玩笑、一段引用，也可能只是思想实验中的假设。会议记录甚至可能把这句话记到了错误的发言人名下。如果把这些差异全部抹平，只留下一个所谓的“事实”，系统反而丢掉了让这句话值得信任的关键信息。</p>
<p>因此，note.md 保存的是<b>主张</b>，而不是没有来源的事实。每条主张都会保留它描述的是谁、由谁提出、来自哪里、在什么时间适用，以及得到了哪一种批准。来源可以帮助 Agent 提出一条候选记忆，却不能代替你批准它。</p>
<p>所以，审核并不是随着模型进步就可以拆掉的临时脚手架。正是这个环节赋予了记忆可信依据。</p>"""),
  ("一个“确认”按钮背后，其实是三种不同的决定", """<p>“记住这件事”“这项外部主张属实”和“你可以据此采取行动”，代表的并不是同一种许可。note.md 会把它们分开：</p>
<table><thead><tr><th>你的决定</th><th>它代表什么</th><th>它不代表什么</th></tr></thead><tbody>
<tr><td>这样记住我</td><td>这条主张准确表达了你的情况。</td><td>它不能证明某项外部事实。</td></tr>
<tr><td>确认事实</td><td>你已根据注明的证据和时间核实了这项外部主张。</td><td>它并不授权 Agent 在现实中采取行动。</td></tr>
<tr><td>允许这种行为</td><td>Agent 可以在注明的目的和边界内采取行动。</td><td>它不代表未来所有情形都获得了许可。</td></tr>
</tbody></table>
<p>按钮上的文字会随决定类型而变化。批准只与当时审核的确切内容绑定；候选内容一旦改变，就必须重新审核。Agent 可以提出建议，却不能自行制造一份来自人的批准。</p>"""),
  ("搜索与记忆承担着不同的任务", """<p>许多系统把记忆当成一种特殊的搜索索引，或干脆把整个文档库都视为记忆。note.md 会有意区分二者。</p>
<table><thead><tr><th></th><th>搜索</th><th>记忆</th></tr></thead><tbody>
<tr><td>要回答的问题</td><td>现有资料中，哪些内容现在可能有帮助？</td><td>这个 Agent 可以依据哪些已批准的主张？</td></tr>
<tr><td>规模</td><td>数千份文档</td><td>少量持久的个人主张</td></tr>
<tr><td>方式</td><td>根据相关性、来源、时间和你的关注程度灵活排序</td><td>根据人物、空间、目的、同意范围和时间进行确定性选择</td></tr>
<tr><td>出错时</td><td>你可以换一个关键词重新搜索</td><td>Agent 可能误解你，甚至越过边界</td></tr>
</tbody></table>
<p>搜索能减轻你的记忆负担：你不必记住所有事情，因为需要时还能找到。记忆则帮助 Agent 与你保持一致：它们只会获得少数由你明确认可的主张。内容相关，并不等于获得了使用许可。</p>"""),
  ("真实 vault 中的数据，说明了人工确认为什么重要", """<p>截至 2026 年 9 月 2 日，一个真实的 note.md vault 中共有 <b>83</b> 条 Agent 提出的候选记忆。vault 所有者保留了 <b>27</b> 条，忽略了 <b>56</b> 条。也就是说，接近三分之二看似合理的建议，并不适合成为长期上下文。</p>
<p>这并不说明 Agent 没有用，反而体现了它最适合承担的角色。几乎每条候选记忆都是由 Agent 发现的，其中包含许多用户自己绝不会主动写进个人资料的有效信号。随后，vault 所有者剔除了玩笑、临时状态、重复内容和过度自信的解读。发现带来了覆盖面，确认建立了信任。</p>"""),
  ("note.md 向你承诺什么", """<ul>
<li><b>任何猜测都不会悄悄变成你的个人画像。</b> 在你作出决定之前，Agent 提出的内容始终只是候选建议。</li>
<li><b>你每次只审核一层含义。</b> 涉及敏感身份、边界或授权的主张不能批量批准。</li>
<li><b>你可以查清每条记忆的来历。</b> 来源、提出者、时间、批准情况和修订历史都会随记忆保留。</li>
<li><b>记忆只会用于获得许可的上下文。</b> 空间、目的、服务提供商和共享规则共同决定 Agent 可以接收哪些内容。</li>
<li><b>你的纠正会不断积累价值。</b> 被忽略的建议会受到抑制，指导信息也可以记录未来 Agent 需要避免的错误。</li>
<li><b>这些资产始终属于你。</b> 权威依据保存在本地、由 Git 跟踪的文件中；易读的 Markdown 视图可以重新生成，其他 Agent 也能使用同一份已批准的上下文。</li>
</ul>"""),
  ("为什么这项工作会越来越轻松", """<p>个人记忆并不是一串永远填不完的资料字段。身份、稳定偏好和边界通常数量有限，它们大多会在早期建立，之后只需偶尔修订。新的决定仍会不断出现，但你可以逐件审核，而不必每次都重写一遍个人经历。</p>
<p>这个循环也会逐步改善。已批准的主张能帮助下一轮发现减少重复，被忽略的候选内容不会反复出现，每次纠正都会提醒未来的 Agent 不要作出同样的假设。目标不是向你提出更多问题，而是把你的注意力留给那些只有你才能回答的少数问题。</p>"""),
  ("选择你想要的记忆契约", """<p>不同的使用方式，适合不同的记忆契约：</p>
<table><thead><tr><th>方式</th><th>适合的情况</th><th>你选择了什么</th></tr></thead><tbody>
<tr><td>自动记忆</td><td>对话是临时的，风险很低</td><td>最省事；由系统决定保留什么</td></tr>
<tr><td>手写个人说明</td><td>你只有几条稳定指令</td><td>最简单直接；由你自己持续更新</td></tr>
<tr><td>note.md Memory</td><td>多个 Agent 跨工具与你长期协作，真实偏好与边界会影响工作</td><td>Agent 负责发现；你保留最终决定、来源、修订历史与场景控制</td></tr>
</tbody></table>
<p>如果你更愿意只向 Agent 提供少量但可信的记忆，而不是接受一份由无声猜测堆积起来的庞大画像，那么 note.md 的方式更适合你。它面向希望获得实用个性化体验、却不愿把“定义自己”的权利交给 AI 的人。</p>
<p>接下来的改进也会遵循同一理念：更准确地从用户选定的来源中发现候选记忆，合理安排审核节奏，让人能够认真判断；结合时间和实际调用情况，提示哪些记忆值得复查；从成功与失败的工作中总结经验，并发布可重复验证的质量指标。这些改进会让整个循环更安静、更有帮助，但不会取消人的确认环节。</p>
<p><b>外部数据库只是保存关于你的陈述。真正值得信任的第二记忆，则会让你记得：它所知道的内容，都曾由你亲自批准。</b></p>"""),
 ],
 "faq": [
  ("note.md 会记录我的所有消息吗？",
